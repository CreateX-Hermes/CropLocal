...
function Stand(props) {
  ...
  let { _id, standName, city, distance, rating, description, ownerName, picture, favorite } =
    props.data;
  ...
  const updateFavoriteList = async () => {
    ...
    await axios.post('http://localhost:8000/user/updateFavoriteStands', { userID, standID: _id });
  };
  ...
  return (
    <NavigationButton
      ...
      onPress={() => navigation.navigate('Seller', { ...props.data })}
    >
      <Image
        source={{ uri: picture }}
        defaultSource={require('../../assets/market_stand.png')}
        style={{ width: '37%', height: '90%', borderRadius: 20 }}
      />
      <View
        style={{
          flex: 2,
          ...
          >
            {isSaved ? (
              <Image
                ...
                style={{ width: 17, height: 17, tintColor: Colors.RATING }}
              />
            ) : (
              <Image
